...
console.log(`\u001b[0m`)
console.log(chalk.red(`© Made By Liankor - ${new Date().getFullYear()}`))
...
manager.on('shardCreate', shard => {
    let embed = new Discord.EmbedBuilder()
        .setTitle(`🆙・Lancement des shards`)
        .setDescription(`une shard vient d'être lancer`)
        .setFields([
            {
                name: "🆔┆ID",
    ...
    shard.on("death", (process) => {
        const embed = new Discord.EmbedBuilder()
            .setTitle(`🚨・Fermeture de la shard ${shard.id + 1}/${manager.totalShards} de façon inattendue`)
            .setFields([
                {
                    name: "🆔┆ID",
                    value: `${shard.id + 1}/${manager.totalShards}`,
        ...
        if (process.exitCode === null) {
            const embed = new Discord.EmbedBuilder()
                .setTitle(`🚨・Shard ${shard.id + 1}/${manager.totalShards} eteinte avec le code d'erreur NULL !`)
                .setFields([
                    {
                        name: "PID",
                        value: `\`${process.pid}\``,
    ...
    shard.on("shardDisconnect", (event) => {
        const embed = new Discord.EmbedBuilder()
            .setTitle(`🚨・Shard ${shard.id + 1}/${manager.totalShards} déconnectée`)
            .setDescription("Vidage de l'événement de fermeture du socket...")
            .setColor(config.colors.normal)
        shardLogs.send({
            username: 'Bot Logs',
            embeds: [embed],
